...
export default class Board {
    ...
    public getLegalMoves(): Array<[number, number]> {
    ...
    public newPiece(p: Hex, pieces: Hex[]): Array<[number, number]> {
      // const onBoard:Array<[number, number]> = []
       ...
       return t.flatMap((el) => el).filter((el) => !this.checkSame(p, el, pieces) );
    ...
    public getJumpMoves(p: Hex, pieces: Hex[]): Array<[number, number]> {
        ...
        for (let i = 0; i < adjPieces.length; i++) {
            ...
            const xInc = pLoc.x - ogLoc.x;
            const yInc = pLoc.y - ogLoc.y;
            const zInc = pLoc.z - ogLoc.z;
            let x = xInc; let y = yInc; let z = zInc;
            while (true) {
    ...
    private floodFill(scurry: Array<[number, number]>, nx: number, ny: number, moves: number, soFar: Array<[number, number]>, pieces: Hex[]) {
        ...
        // this ill need later, but the above works for now
        // if(moves === 0){
        //     this.v.push([nx,ny])
        // }
