...
export function calcHexPoints(x, y, size) {
    const sides = [];
...
export function cube_to_coord(cube) {
    const cx = cube.x;
    const cy = cube.z + (cube.x - (cube.x & 1)) / 2;
    return { x: cx, y: cy };
}
export function coord_to_cube(hex) {
    const cx = hex.x;
    const cz = hex.y - (hex.x - (hex.x & 1)) / 2;
    const cy = -cx - cz;
    return { x: cx, y: cy, z: cz };
}
export function cube_round(cube) {
...
export function hex_to_pixel(x, y, size) {
    const tx = size * 3 / 2 * x;
    const ty = size * Math.sqrt(3) * (y + 0.5 * (x & 1));
    return [tx, ty];
}
export function pixel_to_hex(x, y, size) {
    const q = (2. / 3 * x) / size;
...
export function axial_to_cube(ax, ay) {
    const x = ax;
    const z = ay;
    const y = -x - z;
    return { x, y, z };
...
export function neighbor(x, y, direction) {
    const parity = x & 1;
    const dir = directions[parity][direction];
    return [x + dir[0], y + dir[1]];
}
// end of RedBlob algorithms
//# sourceMappingURL=RedBlob.js.map